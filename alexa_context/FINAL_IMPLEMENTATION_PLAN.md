...
**Search Strategy:** Vertex AI Search (updated May 6)  
...
│                                                                  │
│  ┌────────────────────────────────────────────────────┐        │
│  │  Vertex AI Search (UPDATED May 6, 2026)           │        │
│  │                                                      │        │
│  │  - Vertex AI Search indexes (WW, Coles, Aldi)     │        │
│  │  - 4 weeks historical data per retailer            │        │
│  │  - Returns top 3 candidates per ingredient         │        │
...
### Day 3 (Wednesday, April 23): Vertex AI Search Setup (UPDATED May 6)

**UPDATED APPROACH:** Use Vertex AI Search instead of local embeddings for better accuracy.

**Reason for change:** Simple keyword/embedding search returned irrelevant results (e.g., "chicken breast bulk pack" → "nuggets"). Vertex AI Search provides superior semantic matching.

**Implementation:**

```python
# src/agents/vertex_ai_product_search.py

from google.cloud import discoveryengine_v1beta as discoveryengine

class VertexAIProductSearch:
    """
    Vertex AI Search for semantic product retrieval
    Replaces local embedding approach
    """
    
    def __init__(self):
        self.client = discoveryengine.SearchServiceClient()
        self.search_configs = {
            'woolworths': 'projects/.../locations/.../dataStores/woolworths-products',
            'coles': 'projects/.../locations/.../dataStores/coles-products',
            'aldi': 'projects/.../locations/.../dataStores/aldi-products'
        }
    ...
    def search(self, query, retailer, max_results=3):
        ...
        Args:
            query: Ingredient description (e.g., "lean mince beef 1kg")
            retailer: 'woolworths', 'coles', or 'aldi'
            max_results: Number of results (default 3, was 5)
        
        Returns:
            List of top products with full metadata
        """
        ...
        request = discoveryengine.SearchRequest(
            ...
            # Boost based on promotions and price
            boost_spec=discoveryengine.SearchRequest.BoostSpec(
                condition_boost_specs=[
                    {"condition": "promotion_type: 'yellow_ticket'", "boost": 2.0},
                    {"condition": "promotion_type: 'red_edlp'", "boost": 1.5},
                ]
            )
        )
        ...
        for result in response.results:
            ...
            products.append({
                ...
                'brand': doc.derived_struct_data.get('brand'),
                'price': doc.derived_struct_data.get('price'),
                'unit_price': doc.derived_struct_data.get('unit_price'),
                'pack_size': doc.derived_struct_data.get('pack_size'),
                'promotion': doc.derived_struct_data.get('promotion_type'),
                'relevance_score': result.relevance_score
            })
        
        return products

# Setup: Create Vertex AI Search indexes
def create_vertex_ai_indexes():
    """
    Create 3 Vertex AI Search indexes (Woolworths, Coles, Aldi)
    One index per retailer covering 4 weeks of historical data
    """
    # Implementation: Use gcloud CLI or Python client
    # TODO: Determine if we need separate indexes per week or combined
    pass
```

**Setup Tasks:**
1. Create 3 Vertex AI Search data stores (WW, Coles, Aldi)
2. Index product catalogs with 4 weeks of historical data
3. Configure boost specs for promotions
4. Test search accuracy vs previous approach

**Success Criteria:**
- Improved ingredient matching (no more "nuggets" for "chicken breast")
- Top 3 results contain relevant products >95% of time
